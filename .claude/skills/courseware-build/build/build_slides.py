#!/usr/bin/env python3
"""Generate the Responsible Generative AI Basics slide deck
...
from data_insights import INSIGHTS
...
# GitHub Pages host for the in-class activity web apps
...
def activity_slide(tag,title,desc,steps,build,duration,kicker):
    """One-slide activity brief: scenario, compact workflow strip of SHORT phase
    labels, deliverable band. House rule: the deck shows WHAT the activity covers,
    never the step-by-step — the detailed numbered steps live in the Learner Guide."""
    if len(steps)>6:
        extra=len(steps)-5
        steps=list(steps[:5])+[f"+{extra} more — see the Learner Guide"]
...
SLIDE_MAP={}
def here():
    """True 1-based slide index of the slide just added (cover counts as 1),
    for the Lesson Plan's 'Slides' citations."""
...
SLIDE_MAP["digital_attendance_front"]=here()
...
trainer_slide("YOUR TRAINER",C.TRAINER,"Principal Trainer\nTertiary Infotech Academy Pte Ltd",
 [("Role","Principal Trainer, Tertiary Infotech Academy Pte Ltd"),
  ("Certification / Credentials","PhD; 20+ years of training and industry experience in AI and data."),
  ("Delivers","WSQ courses on responsible AI, generative AI practices, data analytics and machine learning."),
...
content("Let's Know Each Other",[
 ...
 "One thing about AI that genuinely worries you."],kicker="ICE-BREAKER")
...
tile_grid("Skills Framework",[
 ...
 ("Structure","3 Learning Units (LU1–LU3), each mapped to a Learning Outcome.")],
 kicker="COURSE ACCREDITATION",cols=2,size=15)
tile_grid("Course Outline",
 [(f"{t['code']} — {t['title']}", t["subtitle"]) for t in C.TOPICS],
 kicker="3 LEARNING UNITS",cols=1,size=14)
SLIDE_MAP["lesson_plan_slide"]=None
two_col("Lesson Plan — 1 Day",
 [("Morning — 9:30am to 12:10pm",0),
  ("Welcome, ground rules and AM digital attendance",1),
  ("LU1: Ethical Principles of Generative AI (Activities 1–6)",1),
  ("Tea break at 11:05am; lunch 12:10–12:55pm",1)],
 [("Afternoon — 12:55pm to 7:05pm",0),
  ("LU1 concludes; LU2: Privacy Techniques (Activities 7–9)",1),
  ("LU3: Best Practices of Responsible GenAI (Activities 10–14)",1),
  ("Briefing 5:55pm; Assessment: WA (30 min) + Case Study (30 min), 6:05–7:05pm",1),
  ("Timing",0),("45-minute lunch and two 10-minute tea breaks. Assessment is 1 hour in total.",1)],
 kicker="SCHEDULE",lhead="Morning",rhead="Afternoon & Assessment")
SLIDE_MAP["lesson_plan_slide"]=here()
tile_grid("Learning Outcomes",[
 ("LO1 · Ethical Reasoning","Apply ethical reasoning to assess generative AI outputs and guide responsible decision-making in AI implementation."),
 ("LO2 · Privacy Safeguards","Apply privacy safeguards to protect sensitive data used in generative AI through anonymisation and secure storage methods."),
 ("LO3 · Ethical Comparison","Compare generative AI systems for ethical compliance based on intellectual property, privacy and environmental impact.")],
...
SLIDE_MAP["briefing_assessment"]=here()
content("Assessment",[
...
SLIDE_MAP["assessment_front"]=here()
tile_grid("What Is Assessed",
 [(f"{c} — Written Assessment (SAQ)", t) for c,t in C.KNOWLEDGE],
 kicker="KNOWLEDGE STATEMENTS · WA-SAQ",cols=1,size=13)
tile_grid("What Is Assessed",
 [(f"{c} — Case Study", t) for c,t in C.ABILITIES],
 kicker="ABILITY STATEMENTS · CS",cols=1,size=12)
...
def render_insight(kind,spec):
    ...
    if kind=="pillars":
        ...
        # Scale type to the panel height: with 4 rows the cards are ~0.95in tall,
        # so a fixed 16/12pt pair overflows the card. ~11 chars/inch at 11pt.
        hsz = 16 if rows<=2 else (14 if rows==3 else 13)
        bsz = 12 if rows<=2 else (11 if rows==3 else 10)
        head_h = Inches(0.34 if rows>=4 else 0.4)
        top_pad = Inches(0.12 if rows>=4 else 0.18)
        for i,(h,b) in enumerate(items):
            r=i//cols; c=i%cols
            x=int(X0+(cw+gx)*c); y=int(Y0+(chh+gy)*r); col=PALETTE[i%len(PALETTE)]
            rect(s,x,y,cw,chh,LIGHT); rect(s,x,y,Inches(0.1),chh,col)
            txt(s,x+Inches(0.3),int(y+top_pad),cw-Inches(0.55),head_h,[[(h,hsz,col,True)]])
            by=int(y+top_pad+head_h+Inches(0.04))
            txt(s,x+Inches(0.3),by,cw-Inches(0.55),int(y+chh-by-Inches(0.08)),[[(b,bsz,INK,False)]])
        footer(s)
    elif kind=="table":
...
for t in C.TOPICS:
    ...
    for a in acts:
        # House rule: the DECK shows only the short phase labels — the detailed
        # numbered step-by-step belongs in the Learner Guide, never on a slide.
        activity_slide(f"ACTIVITY {a['num']}", a["title"], a["desc"],
                       a["phases"],
                       a["build"], a["duration"], kicker=f"{t['code']} · ACTIVITY {a['num']}")
        SLIDE_MAP[f"activity{a['num']}"]=here()
    tile_grid(f"Activity Links — {t['title']}",
              [(f"Activity {a['num']}: {a['title']}",
                f"{LAB_BASE}{a['path'].split('labs/',1)[1]}"
                + ("   ·   Needs a Gemini API key" if a.get("apikey") else ""))
               for a in acts],
              kicker=f"{t['code']} · OPEN THE ACTIVITIES", cols=1, size=12)
...
tile_grid("What You Achieved",[
 ("LO1 · Ethical Reasoning","Assessed generative AI outputs against ethical principles, exercised professional scepticism and made documented, defensible decisions."),
 ("LO2 · Privacy Safeguards","Applied anonymisation, de-identification and secure-storage measures, and designed an ethical AI data policy."),
 ("LO3 · Ethical Comparison","Compared AI systems on intellectual property, data privacy and environmental impact, and analysed real ethical failures.")],
...
content("Digital Attendance (Mandatory)",[
 ...
 "Scan the QR code with your mobile phone camera and submit your attendance.",
 "A minimum of 75% attendance is required to be eligible for assessment and funding."],kicker="SSG DIGITAL ATTENDANCE")
SLIDE_MAP["digital_attendance_end"]=here()
big_statement("Thank You!","You can now question a confident machine, protect the data you feed it, and compare AI systems on more than accuracy alone.",
